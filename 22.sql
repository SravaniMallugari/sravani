...
Max_Allowed_Books int Check (Max_Allowed_Books <= 100),
 Penalty int Check(Penalty <= 1000))

Create table Book_C(
Book_No int primary key, 
Book_Name varchar(20) not null, 
Category varchar(20) check(category in ('Science', 'Fiction', 'Database', 'RDBMS', 'Others')),
price int )
...
create view display
as
select i.Lib_Issue_Id, b.Book_Name, m.Member_Name, b.Author , i.Issue_date from Book b inner join Lib_Isssue i on b.Book_No = i.Book_No inner join Member m on i.Member_Id = m.Member_ID where (b.Cost between 500 and 750) and b.Category in ('Database' , 'RDBMS')
...
create index index2
on  Lib_Isssue(Member_ID,Book_No)
