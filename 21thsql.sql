...
go
Select count(ContactName) as custcount from Customers where ContactName like 'b%';

go
Select count(ContactName) as custcount from Customers where ContactName like '%s%';

go
Select City,count(ContactName) as noofppl from Customers group by City;

go
Select top 3 City,count(ContactName) as serial from Customers group by City order by serial desc;
go
